...
class DAT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.patch_proj = (
            ...
            if use_conv_patches
            else nn.Sequential(nn.Conv2d(3, dim_stem, patch_size, patch_size, 0), LayerNormProxy(dim_stem))
        )
        ...
        for i in range(3):
            self.down_projs.append(
                nn.Sequential(nn.Conv2d(dims[i], dims[i + 1], 3, 2, 1, bias=False), LayerNormProxy(dims[i + 1]))
                if use_conv_patches
                else nn.Sequential(nn.Conv2d(dims[i], dims[i + 1], 2, 2, 0, bias=False), LayerNormProxy(dims[i + 1]))
            )
    ...
    def load_pretrained(self, state_dict, lookup_22k=None):
        if lookup_22k is not None and not torch.is_tensor(lookup_22k):
            raise TypeError("lookup_22k must be a torch.Tensor when provided")
        ...
        for state_key, state_value in state_dict.items():
            ...
            if m.shape == state_value.shape:
                ...
                if "cls_head" in keys and lookup_22k is not None:
